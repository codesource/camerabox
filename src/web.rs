...
pub fn router(state: Arc<AppState>) -> Router {
    Router::new()
        ...
        .route("/api/network/ap", post(set_ap))
...
async fn network_status(State(state): State<Arc<AppState>>) -> Response {
    let s = net::status().await;
    let ap = state.ap.lock().await.clone();
    // Expose the SSID (to pre-fill the form) but never the AP password.
    Json(json!({
        "interfaces": s.interfaces,
        "profiles": s.profiles,
        "ap": { "ssid": ap.ssid, "ip": ap.ip_cidr },
    }))
    .into_response()
}
...
async fn network_hotspot(State(state): State<Arc<AppState>>, Json(r): Json<IfaceReq>) -> Response {
    let ap = state.ap.lock().await.clone();
    net_result(net::start_hotspot(&r.iface, &ap).await)
}

#[derive(Deserialize)]
struct ApReq {
    ssid: String,
    /// Omitted/empty keeps the current password.
    #[serde(default)]
    password: Option<String>,
    /// Omitted/empty keeps the current AP IP/subnet.
    #[serde(default)]
    ip: Option<String>,
}

/// Save the hotspot SSID/password/IP, persist them, and apply live if the AP is up.
async fn set_ap(State(state): State<Arc<AppState>>, Json(r): Json<ApReq>) -> Response {
    let iface = match net::primary_wifi() {
        Some(i) => i,
        None => {
            return (StatusCode::BAD_REQUEST, Json(json!({ "error": "no wireless interface" })))
                .into_response()
        }
    };
    let mut ap = state.ap.lock().await;
    let new = crate::config::ApConfig {
        ssid: r.ssid.trim().to_string(),
        password: match r.password {
    ...
    if let Err(e) = net::validate_ap(&new) {
        return (StatusCode::BAD_REQUEST, Json(json!({ "error": e }))).into_response();
    }
    if let Err(e) = new.save(&state.ap_path) {
        return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({ "error": e.to_string() })))
            .into_response();
    }
    *ap = new.clone();
    drop(ap);
    net_result(net::apply_ap(&iface, &new).await)
}
...
async fn set_hostname(State(state): State<Arc<AppState>>, Json(r): Json<HostnameReq>) -> Response {
    let ap_ip = {
        let ap = state.ap.lock().await;
        ap.ip_cidr.split('/').next().unwrap_or("192.168.4.1").to_string()
    };
...
function camCard(c){
 ...
 return '<div class="card cam-card hov">'+
  ...
  '<div class="row" style="margin-top:14px">'+(c.running&&c.stream_url?'<a class="btn primary sm" href="'+esc(surl)+'" target="_blank">'+ic('play',16)+t('preview')+'</a>':'<button class="btn sm" disabled>'+ic('play',16)+t('preview')+'</button>')+(c.stream_url?'<button class="btn sm" onclick="copyLink(\''+esc(surl)+'\')">'+ic('link',16)+t('copy_link')+'</button>':'')+'</div>'+
...
function checkUpdate(btn){ btn.disabled=true; var o=btn.innerHTML; btn.textContent=t('checking'); var inf=el('updinfo'); if(inf)inf.textContent='';
 getJSON('/api/update/check').then(function(d){ btn.disabled=false; btn.innerHTML=o; if(!inf)return;
  if(!d||d.error){ inf.textContent=t('update_check_failed'); return; }
  if(d.update_available){ inf.innerHTML=esc(tf('update_avail',d.latest))+' <button class="btn primary sm" style="margin-left:8px" onclick="doUpdate(this)">'+t('update_now')+'</button>'; }
  else { inf.textContent=tf('up_to_date',d.current||''); }
 }).catch(function(){ btn.disabled=false; btn.innerHTML=o; if(inf)inf.textContent=t('update_check_failed'); }); }
function doUpdate(btn){ if(!confirm(t('update_confirm')))return; btn.disabled=true; btn.textContent=t('updating'); toast(t('updating'));
 api('POST','/api/update').then(function(r){return r.json().catch(function(){return{};}).then(function(d){return{ok:r.ok,d:d};});}).then(function(x){
